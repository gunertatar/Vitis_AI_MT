...
def train():
    ...
    if args.cuda:
        ...
        net = net.to(device)
        weight_det = weight_det.to(device)
        weight_lane = weight_lane.to(device)
        weight_seg = weight_seg.to(device)
        weight_drivable = weight_drivable.to(device)
        weight_depth = weight_depth.to(device)
    ...
    # SSD ÖZELLİK ÇIKARICI OLARAK KULLANILMIŞTIR.(AtssLoss(solver)) 
    ...
    lovasz_softmax_loss_seg = LovaszSoftmaxLoss(weight=[1.] * solver['seg_classes'], device = device)
    lovasz_softmax_loss_lane = LovaszSoftmaxLoss(weight=[1.] * solver['seg_lane'], device = device)
    lovasz_softmax_loss_drivable = LovaszSoftmaxLoss(weight=[1.] * solver['seg_drivable'], device = device)
    ...
    if train_det:
        data_loader_det = data.DataLoader(dataset_det, int(1),
                                          num_workers=4,
                                          shuffle=True, collate_fn=detection_collate,
                                          pin_memory=True)

    if train_seg:
        data_loader_seg = data.DataLoader(dataset_seg, int(1),
                                          num_workers=4,
                                          shuffle=True, collate_fn=segmentation_collate,
                                          pin_memory=True)

    if train_drivable:
        data_loader_drivable = data.DataLoader(dataset_drivable, int(1),
                                               num_workers=4,
                                               shuffle=True, collate_fn=segmentation_collate,
                                               pin_memory=True)
    if train_depth:
        data_loader_depth = data.DataLoader(dataset_depth, int(1),
                                            num_workers=4,
                                            shuffle=True, collate_fn=depth_collate,
                                            pin_memory=True)
    if train_lane:
        data_loader_lane = data.DataLoader(dataset_lane, int(1),
                                           num_workers=4,
                                           shuffle=True, collate_fn=segmentation_collate,
                                           pin_memory=True)
    ...
    for iteration in range(args.start_iter, solver['max_iter']):
        ...
        for iter_round in range(1):
            ...
            if train_drivable:
                ...
                if args.cuda:
                    device = torch.device("cuda")
                    images_drivable = Variable(images_drivable.to(device))
                    with torch.no_grad():
                        drivable = Variable(drivable.to(device))
                else:
