...
def test_net(save_folder, net, device, ids, detect, transform, thresh=0.01, has_ori=False):
    ...
    for i in range(num_images):
        ...
        if drivable_data is not None:
            ...
            save_drivable_file = os.path.join(save_drivable_root, img_id[1] + '.png') # Ben ekledimmmmmmm--------------------------------------------------------
        ...
        if lane_data is not None:
            ...
            save_lane_file = os.path.join(save_lane_root, img_id[1] + '.png')    # Ben ekledimmmmmmm--------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    # EKLEDİĞİM KISIMLAR ALT -------------------------------------------------------------------------------------------------------------------------------------------------------------
    print("MODEL SUMMARY")
    from torchinfo import summary
    import torch.nn.init as init
    a = summary(net, (1, 3, 320, 512)) 
    print(a) 
    #----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    torch.onnx.export(net, x, 'MTNet.onnx', input_names=input_names, output_names=output_names)
    
    from torchviz import make_dot
    y = torch.rand(1, 3, 320, 512, requires_grad=True)
    make_dot(x, params=dict(net.named_parameters())).render("rnn_torchviz", format="png")

    # EKLEDİĞİM KISIMLAR ÜST ------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if args.eval:
        ...
        ids = list()
        for line in open("/home/xilinx/Vitis-AI/pt_multitaskv3_mixed_320_512_25.44G_2.5/code/test/demo_data/demo_list.txt"):
            ids.append(("demo_data/images/", line.strip()))
        detect = Detect(det_classes, 0, 200, args.confidence_threshold,
                        0.45)  # num_classes, backgroung_label, top_k, conf_threshold, nms_threshold
